...
public class MDP_DynamicPragraming {
	...
	private void mdp() {
		...
		//判断当前应该分配几个
		double[] totalweight  = new double[AllunAssiNum];
		int oneassNum =0;
		
		for(int assNum = 0; assNum < AllunAssiNum; assNum++){
			...
			for(int j=0; j<assNum; j++){
				selectweight += Sampletable[1][j];
			}
			
			DecimalFormat df = new DecimalFormat("#.####");

			double ap = (double)Sampletable[1][assNum]/(double)totalpuweight;
			double eu = (double)assNum/(double)(AllunAssiNum);
			
			totalweight[assNum] = Double.parseDouble(df.format(ap -eu));
			
			
			//迭代循环计算所有剩余分配次数的最大值
			for(int t=0; t< restTime; t++){
				totalpuweight = totalpuweight - selectweight;
				
				
				double tempweight = totalweight[assNum];
...
//		}
	...
	public static void main(String[] args) throws IOException {
		...
		MDP_DynamicPragraming mpd = new MDP_DynamicPragraming();
